...
impl Ed25519KeyMaterial {
    ...
    pub fn from_mnemonic(mnemonic: &str) -> Result<Ed25519KeyMaterial, JsError> {
        ...
        let mnemonic_obj =
            szdt_core::mnemonic::Mnemonic::parse(mnemonic).map_err(core_error_to_js_error)?;
    ...
    pub fn from_public_key(public_key: &[u8]) -> Result<Ed25519KeyMaterial, JsError> {
        ...
        let inner =
            CoreKeyMaterial::try_from_public_key(public_key).map_err(core_error_to_js_error)?;
        Ok(Self { inner })
    }
