...
impl CborSeqReader {
    ...
    pub fn read_memo(&mut self) -> Result<Memo, JsError> {
        ...
        let core_memo: szdt_core::memo::Memo = reader.read_block()
            .map_err(|e| match e {
    ...
    pub fn read_raw(&mut self) -> Result<Vec<u8>, JsError> {
        ...
        let value: Value = reader.read_block()
            .map_err(|e| match e {
                CoreError::Eof => JsError::new("End of sequence reached"),
                _ => JsError::new(&e.to_string()),
            })?;
        ...
        let cbor_bytes = serde_ipld_dagcbor::to_vec(&value).map_err(|e| JsError::new(&e.to_string()))?;
        Ok(cbor_bytes)
    }
...
impl CborSeqWriter {
    ...
    pub fn write_raw(&mut self, cbor_data: &[u8]) -> Result<(), JsError> {
        ...
        let _value: Value = serde_ipld_dagcbor::from_slice(cbor_data).map_err(|e| JsError::new(&e.to_string()))?;
    ...
    pub fn write_object(&mut self, js_value: &JsValue) -> Result<(), JsError> {
        ...
        let value: Value = serde_wasm_bindgen::from_value(js_value.clone()).map_err(|e| JsError::new(&e.to_string()))?;
...
pub fn parse_cbor(data: &[u8]) -> Result<JsValue, JsError> {
    let value: Value = serde_ipld_dagcbor::from_slice(data).map_err(|e| JsError::new(&e.to_string()))?;
    let js_value = serde_wasm_bindgen::to_value(&value).map_err(|e| JsError::new(&e.to_string()))?;
    Ok(js_value)
}
